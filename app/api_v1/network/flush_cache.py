...
request_schema = RequestSchema(
    only=('request_number', 'creation_date', 'jobid', 'status', 'result'))
...
class AkamaiFlushCacheListResource(Resource):
    ...
    def post(self):
        ...
        try:
            cpcodes = [AkamaiFlushCache(cpcode=cpcode)
                       for cpcode in request_data['cpcodes']]
            emails = [RequestorEmails(email=email)
                      for email in request_data['request']['notify']]
...
class AkamaiFlushCacheResource(Resource):
    def get(self, request_number):
        requests = Requests.query.filter_by(
            request_number=request_number).first_or_404()
        result = request_schema.dump(requests)
        return result


api.add_resource(AkamaiFlushCacheListResource, '/v1/requests/flushcache/')
api.add_resource(AkamaiFlushCacheResource,
                 '/v1/requests/flushcache/<string:request_number>')
